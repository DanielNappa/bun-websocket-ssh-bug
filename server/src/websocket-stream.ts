...
export class WebSocketServerStream extends BaseStream {
  ...
  constructor(websocket: connection) {
    ...
    websocket.on('message', (data) => {
      if (data.type === 'binary') {
        console.log(`[WebSocketServerStream] Received binary message, size: ${data.binaryData?.length || 'unknown'}`);
    ...
    websocket.on('close', (code?: number, reason?: string) => {
      console.log(`[WebSocketServerStream] Connection closed, code: ${code}, reason: ${reason}`);
    ...
    websocket.on('error', (error: Error) => {
      console.error(`[WebSocketServerStream] WebSocket error: ${error.message}`);
      this.onError(error);
    });
  }
  ...
  async write(data: Buffer): Promise<void> {
    ...
    console.log(`[WebSocketServerStream] Sending binary data, size: ${data.length}`);
  ...
  async close(error?: Error): Promise<void> {
    ...
    if (!error) {
      ...
      console.log(`[WebSocketServerStream] Closing connection with error: ${error.message}`);
